...
def score_song(user_prefs: Dict, song: Dict) -> Tuple[float, List[str]]:
    ...
    if decade_match:
        reasons.append(f"release decade match (+{decade_points:.3f})")
    if context_match:
        reasons.append(f"context match (+{context_points:.3f})")
    if tag_match >= 0.50:
        reasons.append(f"detailed mood tag overlap (+{tag_points:.3f})")
    ...
    if numeric_closeness.get("popularity", 0.0) >= 0.70:
        reasons.append(f"popularity close to target (+{popularity_points:.3f})")
    if numeric_closeness.get("vocal_presence", 0.0) >= 0.70:
        reasons.append(f"vocal presence close to target (+{vocal_points:.3f})")
    if numeric_closeness.get("instrumental_focus", 0.0) >= 0.70:
        reasons.append(f"instrumental focus close to target (+{instrumental_points:.3f})")
    if numeric_closeness.get("replay_value", 0.0) >= 0.70:
        reasons.append(f"replay value close to target (+{replay_points:.3f})")
    if not reasons:
...
def _apply_diversity_rerank(
    ...
    while remaining and len(selected) < k:
        ...
        for song, base_score, explanation in remaining:
            ...
            if artist_penalty > 0:
                reason_parts.append(f"diversity penalty: repeated artist (-{artist_penalty:.3f})")
            if genre_penalty > 0:
                reason_parts.append(f"diversity penalty: repeated genre (-{genre_penalty:.3f})")
