...
def test_score_song_rewards_decade_context_and_tag_overlap():
    ...
    assert score > 0.95
    assert "release decade match" in joined_reasons
    assert "context match" in joined_reasons
    assert "detailed mood tag overlap" in joined_reasons
    assert "popularity close to target" in joined_reasons
